...
async function uploadR2(manifestPath, { bucket, prefix = "", token, accountId, concurrency = 8 }) {
  ...
  const r2Dir = path.resolve(path.dirname(manifestPath), manifest.r2.dir.startsWith("/") ? "" : "", manifest.r2.dir);
  // Resolve r2Dir relative to manifest if it's relative
  const blobsDir = path.isAbsolute(manifest.r2.dir)
    ? manifest.r2.dir
    : path.resolve(path.dirname(manifestPath), "..", path.basename(manifest.r2.dir));
